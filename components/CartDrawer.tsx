...
const CartDrawer: React.FC<CartDrawerProps> = ({ 
  ...
  const handleCheckoutClick = () => {
    ...
    if (!selectedCity) {
      alert(lang === 'ar' ? 'الرجاء اختيار مدينة التوصيل' : 'Please select a delivery city');
      return;
    }
        ...
        <div className="flex-grow overflow-y-auto p-4 space-y-3 bg-gray-50/50">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center px-10 opacity-40">
              ...
              <p className="text-lg font-black">{lang === 'ar' ? 'السلة فارغة' : 'Your basket is empty'}</p>
            </div>
          ) : (
            cart.map(item => (
              <div key={item.id} className="flex gap-3 p-3 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
                <img src={item.image} className="w-16 h-16 object-cover rounded-xl border border-gray-100" alt="" />
                <div className="flex-grow min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-black text-orange-600 text-sm">{(item.price * item.quantity).toFixed(2)} JD</span>
                  </div>
                </div>
              </div>
            ))
        ...
        <div className="shrink-0 bg-white border-t border-gray-100 p-6 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] space-y-4">
          {cart.length > 0 && (
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className={`text-[10px] font-black text-gray-400 uppercase tracking-widest block ${lang === 'ar' ? 'text-right mr-1' : 'ml-1'}`}>
                ...
                <div className="flex items-center" dir="ltr">
                  ...
                  <input
                    ...
                    className="flex-1 min-w-0 bg-white border border-gray-200 px-3 py-2.5 rounded-r-xl text-sm font-black tracking-widest focus:ring-2 focus:ring-[#266041] focus:outline-none transition-all"
                  />
                </div>
              </div>
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-gray-50 border border-gray-200 px-3 py-2.5 rounded-xl text-sm font-black text-[#266041] focus:ring-2 focus:ring-[#266041] focus:outline-none appearance-none"
                >
                  {JORDAN_CITIES.map(c => (
          ...
          <div className="space-y-1 px-1">
            ...
            <div className="flex justify-between text-[10px] font-bold text-gray-400">
              ...
              <span className={deliveryFee === 0 ? 'text-green-600' : ''}>{deliveryFee === 0 ? (lang === 'ar' ? 'مجاني' : 'FREE') : `${deliveryFee.toFixed(2)} JD`}</span>
            </div>
            <div className="pt-2 border-t border-dashed border-gray-200 flex justify-between items-center">
              <span className="text-sm font-black text-gray-800">{lang === 'ar' ? 'الإجمالي' : 'Total'}</span>
              <span className="text-xl font-black text-orange-600">{total.toFixed(2)} JD</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-2.5">
            <button 
              ...
              className="w-full bg-[#266041] hover:bg-[#1a4a32] text-white py-3.5 rounded-xl font-black text-base shadow-lg disabled:opacity-40 transition-all flex items-center justify-center gap-3"
            >
            ...
            <button 
              ...
              className="w-full bg-white border-2 border-green-500 text-green-600 py-2.5 rounded-xl font-black text-xs hover:bg-green-50 transition-all flex items-center justify-center gap-2"
            >
